...
<script type="text/javascript">
	function submit(state){
		$("#fm").form("submit",{
			url:'${pageContext.request.contextPath}/taskExcute/UpdateTaskType.action?state='+state,
			onSubmit:function(){
	...
	$(function(){
		$.post("${pageContext.request.contextPath}/History/findByFlowInfo.action",{taskId:'${param.taskId}'},function(result){
			$("#leavePerson").val(result.current_operator);
			$("#leaveDays").val(result.priority);
			$("#leaveReason").val(result.product_type);
		},"json");
	});
</script>
</head>
	...
	<form id="fm" method="post">
		<table cellspacing="8px">
			...
			<tr>
				<td valign="top">批注：</td>
				<td colspan="4">
					<textarea id="remarks" name="comment" rows="2" cols="49" class="easyui-validatebox" required="true"></textarea>
				</td>
			</tr>
			<tr>
				<td>
					<input type="hidden" name="taskId" value="${param.taskId}"/>
					<input type="hidden" name="username" value="${currentMemberShip.staff_name }"/>
				</td>
				<td colspan="4">
